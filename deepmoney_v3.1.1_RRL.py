...
def main(_):
    ...
    if ensemble:
       ...
       r.write("\n ensemble, gradual, layers, hidden_size, batch_size, iteration\n" +
               str(config[0].ensemble) + "," + str(config[0].gradual) + "," + str(config[0].num_layers) + "," + str(config[0].hidden_size) + "," +
       ...
       for i in range(tot_num_thread):
           ...
           piacc = cal_piacc(res[i]["std"].values, res[i]["index_pred_" + str(i)].values, res[i]["iindex_real"].values,
                             res[i]["loss_profits_" + str(i)].values)
           r.write(
               str(str(config[i].step_interval) + "," + config[i].num_steps) + "," +
       ...
       r.write("\n ensemble, gradual, time steps, interval, layers, hidden_size, batch_size, iteration\n" +
               ...
               str(config.step_interval) + "," + str(config.num_layers) + "," + str(config.hidden_size1) + "," +
