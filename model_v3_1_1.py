...
"""Index predicting Many-to-Many LSTM model.
 ...
 the difference from v3.1
 RRL input:
    [0 : - predict_term / inrweval)  - target price changes from base
    [- predict_term / inrweval :  ]  - predicted price changes from base
...
def model_fn(features, labels, mode, params):
    ...
    diff = []
    diff.append(tf.subtract(target_prices[:, :-int(config.predict_term/config.step_interval), 0], base_prices[:, :-int(config.predict_term/config.step_interval), 0]))
    diff.append(tf.subtract(predict_prices[:, -int(config.predict_term / config.step_interval):, 0],
                        base_prices[:, -int(config.predict_term / config.step_interval):, 0]))
    diff = tf.reshape(tf.concat(diff, axis=1), [batch_size, num_steps, 1])
